...
const AirbagProtectionPage = ({ variant = 'grid' }) => {
  ...
  useEffect(() => {
    fetch("/Product-details/Airbag_Protection.json")
      .then((res) => res.json())
      .then((data) => {
        const allProducts = [
          ...(data.airbag_vests || []),
          ...(data.back_protectors || []),
          ...(data.other_products || [])
        ];
  ...
  return (
    <div style={{ width: '100%', background: '#f7f8fa', padding: '48px 0' }}>
      <h2 style={{ textAlign: 'center', fontSize: 32, fontWeight: 800, marginBottom: 32, color: '#222' }}>Airbag & Protection</h2>
      <div style={containerStyle}>
        {products.map((product, idx) => {
          ...
          const imgSrc = firstImage
            ...
            : null;
          return (
          <div key={idx} className="airbag-card" style={cardStyle}>
            {imgSrc && (
              <Link to="/product" state={{ product, source: 'Airbag-Protection' }}>
                <img
                  src={imgSrc}
                  alt={product.name}
            ...
            <div style={{ fontWeight: 500, fontSize: 16, marginBottom: 8, color: '#27ae60' }}>{product.brand}</div>
            ...
            <div style={{ color: '#888', fontSize: 14, marginBottom: 4 }}>Prix Conseillé : {product.price && (product.price.recommended || product.price)}</div>
            <div style={{ color: '#e53935', fontWeight: 700, fontSize: 24, marginBottom: 16 }}>{product.price && (product.price.web || product.price)}</div>
            <Link
              to="/product"
